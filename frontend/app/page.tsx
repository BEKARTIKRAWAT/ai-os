    ...
    >
      ...
      {dragOver && (
        <div className="absolute inset-0 z-50 flex items-center justify-center border-4 border-dashed" style={{ background: "rgba(124,58,237,0.1)", borderColor: "var(--accent)" }}>
          <div className="text-center rounded-3xl p-8 md:p-12 shadow-2xl mx-4" style={{ background: "var(--bg-primary)" }}>
            <div className="text-5xl md:text-7xl mb-4">📁</div>
            <div className="text-xl md:text-2xl font-bold mb-2">Drop your file here</div>
            ...
            <div className="space-y-2">
              {THEMES.map(t => (
                ...
                >
                  ...
                  {theme === t.id && <span className="text-lg">✓</span>}
                </button>
              ))}
            </div>
          </div>
      ...
      >
        <div className="flex flex-col h-full p-4 w-72 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-2 mb-6">
            ...
            {isMobile && (
              <button onClick={() => setShowSidebar(false)} className="w-8 h-8 flex items-center justify-center rounded-xl" style={{ background: "var(--bg-tertiary)", color: "var(--text-secondary)" }}>✕</button>
            )}
          </div>

          {/* New Chat */}
          <button
            onClick={() => { setMessages([]); setSessionId(null); setTotalTokens(0); if (isMobile) setShowSidebar(false); }}
          ...
          <div className="mb-4">
            ...
            <div className="space-y-0.5">
              {Object.entries(AGENTS).map(([key, val]) => (
                <div key={key} className="flex items-center gap-3 px-3 py-2 rounded-xl text-xs transition-all cursor-default" style={{ background: currentAgent === key ? "var(--accent)" + "20" : "transparent", color: currentAgent === key ? "var(--accent)" : "var(--text-secondary)" }}>
          ...
          <div className="relative mb-3">
            ...
            <input type="text" placeholder="Search chats..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-8 pr-3 py-2 text-xs rounded-xl outline-none" style={{ background: "var(--bg-tertiary)", border: "1px solid var(--border)", color: "var(--text-primary)" }} />
          </div>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide space-y-0.5">
            ...
            {filtered.map((s, i) => (
              <div key={i} onClick={() => loadHistory(s.session_id)} className="group flex items-start gap-2 px-3 py-2.5 rounded-xl cursor-pointer transition-all active:scale-95" style={{ background: sessionId === s.session_id ? "var(--bg-tertiary)" : "transparent" }}>
                ...
                <button onClick={e => { e.stopPropagation(); deleteSession(s.session_id); }} className="opacity-0 group-hover:opacity-100 text-xs transition-all flex-shrink-0" style={{ color: "var(--text-secondary)" }}>✕</button>
              </div>
            ))}
          </div>
          ...
          <div className="pt-3 mt-2 space-y-1" style={{ borderTop: "1px solid var(--border)" }}>
            ...
            <button onClick={() => setShowThemePicker(true)} className="flex items-center gap-2 w-full px-3 py-2 rounded-xl text-xs transition-all active:scale-95" style={{ color: "var(--text-secondary)", background: "var(--bg-tertiary)" }}>
              <span>{currentTheme.icon}</span><span>{currentTheme.name} Theme</span><span className="ml-auto">↗</span>
            </button>
            <a href="/analytics" className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs transition-all" style={{ color: "var(--text-secondary)" }}>{"📊"} Analytics</a>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {/* Header */}
        <header className="flex items-center justify-between px-3 md:px-4 py-3 flex-shrink-0" style={{ background: "var(--header)", borderBottom: "1px solid var(--border)" }}>
          <div className="flex items-center gap-2 md:gap-3">
            <button onClick={() => setShowSidebar(!showSidebar)} className="w-9 h-9 flex items-center justify-center rounded-xl transition-all active:scale-95" style={{ color: "var(--text-secondary)", background: "var(--bg-tertiary)" }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm hidden sm:block">AI-OS</span>
            </div>
          </div>

          <div className="flex items-center gap-1.5 px-2 md:px-3 py-1.5 rounded-full text-xs font-medium" style={{ background: "var(--bg-tertiary)", border: "1px solid var(--border)" }}>
          ...
          <div className="flex items-center gap-1 md:gap-2">
            {messages.length > 0 && (
              <button onClick={() => { setMessages([]); setSessionId(null); }} className="text-xs px-2 md:px-3 py-1.5 rounded-xl transition-all active:scale-95" style={{ color: "var(--text-secondary)", background: "var(--bg-tertiary)" }}>
                <span className="hidden sm:block">Clear</span>
                <span className="sm:hidden">✕</span>
              </button>
            )}
            <button onClick={() => setShowThemePicker(true)} className="text-xs px-2 md:px-3 py-1.5 rounded-xl transition-all active:scale-95" style={{ color: "var(--text-secondary)", background: "var(--bg-tertiary)" }}>
              {currentTheme.icon}
          ...
          {messages.length === 0 ? (
              ...
              <div className="max-w-2xl w-full">
                <div className="text-center mb-8">
                  ...
                  <p className="text-sm md:text-base" style={{ color: "var(--text-secondary)" }}>
                    AI-OS with 7 specialized agents — ready to help!
                  </p>
                </div>

                <div className="flex flex-wrap justify-center gap-2 mb-6">
                  {Object.entries(AGENTS).map(([key, val]) => (
                    <div key={key} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)", color: "var(--accent)" }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                  {SUGGESTIONS.map((s, i) => (
                    <button key={i} onClick={() => sendMessage(s.text)} className="flex items-start gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl text-left transition-all hover:scale-[1.02] active:scale-[0.98]" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)" }}>
                      <span className="text-xl md:text-2xl flex-shrink-0">{s.icon}</span>
                      <div>
                        <div className="text-xs md:text-sm leading-relaxed" style={{ color: "var(--text-primary)" }}>{s.text}</div>
                ...
                <div className="mt-6 p-3 md:p-4 rounded-xl md:rounded-2xl" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)" }}>
                  <div className="flex flex-wrap gap-x-4 gap-y-2 justify-center text-xs" style={{ color: "var(--text-secondary)" }}>
            ...
            <div className="max-w-3xl mx-auto px-3 md:px-4 py-4 md:py-6 space-y-4 md:space-y-6">
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex gap-2 md:gap-3 group ${isUser ? "justify-end" : "justify-start"}`}>
                    {!isUser && (
                      <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg md:rounded-xl flex items-center justify-center text-xs md:text-sm font-bold flex-shrink-0 mt-0.5 text-white shadow-md" style={{ background: "linear-gradient(135deg, var(--accent), var(--accent-2))" }}>
                        {ma.icon}
                      </div>
                    )}
                    <div className={`max-w-[88%] md:max-w-[85%] flex flex-col gap-1 ${isUser ? "items-end" : "items-start"}`}>
                      <div className={`flex items-center gap-1.5 md:gap-2 px-1 ${isUser ? "flex-row-reverse" : ""}`}>
                      ...
                      <div
                        className={`relative rounded-xl md:rounded-2xl px-3 md:px-4 py-2.5 md:py-3 text-sm leading-relaxed ${isUser ? "rounded-tr-sm" : "rounded-tl-sm shadow-sm"}`}
                      ...
                      >
                        {msg.fileInfo && (
                          <div className="flex items-center gap-2 mb-2 pb-2 text-xs" style={{ borderBottom: "1px solid var(--border)", color: "var(--text-secondary)" }}>
                            <span>📄</span><span className="font-medium truncate max-w-32">{msg.fileInfo.name}</span><span>•</span><span>{(msg.fileInfo.size / 1024).toFixed(1)}KB</span>
                          </div>
                        )}
                        {msg.image_base64 && (
                          <div className="mb-3">
                        ...
                        {!isUser ? (
                          <ReactMarkdown components={{
                            code({ className, children, ...props }: any) {
                              ...
                              if (match) return (
                                <div className="my-2 md:my-3 rounded-lg md:rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
                                  <div className="flex items-center justify-between px-3 md:px-4 py-2" style={{ background: "var(--bg-tertiary)", borderBottom: "1px solid var(--border)" }}>
                                    <div className="flex items-center gap-2">
                                      <div className="hidden sm:flex gap-1"><div className="w-2 h-2 rounded-full bg-red-400"></div><div className="w-2 h-2 rounded-full bg-yellow-400"></div><div className="w-2 h-2 rounded-full bg-green-400"></div></div>
                                    ...
                                    <div className="flex gap-1 md:gap-2">
                                      ...
                                      {(match[1] === "python" || match[1] === "javascript" || match[1] === "js") && (
                                        <button onClick={() => runCode(cs, match[1], msg.id)} disabled={runningCode === ok} className="text-xs px-2 md:px-3 py-1 rounded-lg text-white disabled:opacity-50" style={{ background: "var(--accent)" }}>
                                          {runningCode === ok ? "..." : "▶ Run"}
                                        </button>
                                      )}
                                    </div>
                                  ...
                                  {codeOutputs[ok] && (
                                    <div className="px-3 md:px-4 py-3" style={{ background: "#0d0d0d", borderTop: "1px solid var(--border)" }}>
                            ...
                            h1: ({ children }) => <h1 className="text-lg md:text-xl font-bold mb-2 pb-2" style={{ borderBottom: "1px solid var(--border)" }}>{children}</h1>,
                            h2: ({ children }) => <h2 className="text-base font-bold mb-2 mt-3">{children}</h2>,
                            h3: ({ children }) => <h3 className="text-sm font-semibold mb-1.5 mt-2">{children}</h3>,
                            strong: ({ children }) => <strong className="font-semibold">{children}</strong>,
                            blockquote: ({ children }) => <blockquote className="border-l-4 pl-3 my-2 italic" style={{ borderColor: "var(--accent)", color: "var(--text-secondary)" }}>{children}</blockquote>,
                    ...
                    {isUser && <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg md:rounded-xl flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5 shadow-md" style={{ background: "linear-gradient(135deg, #374151, #111827)" }}>K</div>}
                  </div>
                );
              })}
              {loading && (
                <div className="flex gap-2 md:gap-3">
                  <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg md:rounded-xl flex items-center justify-center text-xs md:text-sm font-bold flex-shrink-0 text-white shadow-md" style={{ background: "linear-gradient(135deg, var(--accent), var(--accent-2))" }}>{agent.icon}</div>
                  <div className="rounded-xl md:rounded-2xl rounded-tl-sm px-3 md:px-4 py-2.5 md:py-3 shadow-sm" style={{ background: "var(--bubble-ai)", border: "1px solid var(--border)" }}>
        ...
        {/* File Preview */}
        {selectedFile && (
          <div className="mx-3 md:mx-4 mb-2 px-3 md:px-4 py-2 rounded-xl flex items-center justify-between" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)" }}>
            ...
            <button onClick={() => setSelectedFile(null)} className="text-sm w-6 h-6 flex items-center justify-center rounded-lg flex-shrink-0 ml-2" style={{ color: "var(--text-secondary)", background: "var(--bg-tertiary)" }}>✕</button>
          </div>
        )}

        {/* Input */}
        <div className="px-3 md:px-4 pb-3 md:pb-4 pt-2 flex-shrink-0" style={{ background: "var(--header)" }}>
          <div className="max-w-3xl mx-auto">
            <div className="flex items-end gap-2 rounded-xl md:rounded-2xl px-3 py-2.5 transition-all shadow-sm" style={{ background: "var(--input-bg)", border: "1px solid var(--border)" }}>
              <button
                ...
                className="flex-shrink-0 w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-lg md:rounded-xl transition-all active:scale-90"
              ...
              <textarea
                ...
                placeholder={selectedFile ? `Ask about ${selectedFile.name}...` : "Message AI-OS..."}
                value={input}
                rows={1}
              ...
              />
              <button onClick={startVoice} className="flex-shrink-0 w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-lg md:rounded-xl transition-all active:scale-90" style={{ color: isListening ? "red" : "var(--text-secondary)", background: isListening ? "rgba(255,0,0,0.1)" : "transparent" }}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
              </button>
              <button
                onClick={() => sendMessage()}
                disabled={loading || (!input.trim() && !selectedFile)}
                className="flex-shrink-0 w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-lg md:rounded-xl transition-all text-white active:scale-90"
              ...
              >
                {loading ? <div className="w-3 h-3 md:w-3.5 md:h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin"></div> : <svg className="w-3 h-3 md:w-3.5 md:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 12h14M12 5l7 7-7 7" /></svg>}
              </button>
            </div>
            <p className="text-center text-xs mt-2" style={{ color: "var(--text-secondary)", opacity: 0.6 }}>AI-OS may make mistakes — verify important information</p>
          </div>
        </div>
      </div>
    </div>
